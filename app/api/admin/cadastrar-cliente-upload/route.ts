...
// Senha simples para acesso direto (em produção usar sistema de auth real)
  ...
  try {
    ...
    try {
      ...
      return NextResponse.json({
        ...
        userId: newUser.id,
        email: newUser.email,
        administradora: administradora || 'Não identificada',
      })
    } catch (error: any) {
    ...
    return NextResponse.json(
      { error: 'Erro ao cadastrar cliente: ' + (error.message || 'Erro desconhecido') },
      { status: 500 }
    )
  }
}
